Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/e3b12775ef605a142855cac6e87aabb1"
{
	Properties
	{
...
#define PROP_BASECOLOR_STEP 0.161
#define PROP_BASESHADE_FEATHER 0.071
#define PROP_SHADECOLOR_STEP 0.076
#define PROP_1ST2ND_SHADES_FEATHER 0.071
...
#define PROP_RIMLIGHT_POWER 0.65
#define PROP_RIMLIGHT_INSIDEMASK 0.4
...
#define PROP_OUTLINETINTMIX 0.5
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0.2) * float(1) / 100) * outlineMask * distanceOffset;
				if (float(1) == 2)
				{
			...
			{
				float MainColorFeatherStep = float(0.161) - float(0.071);
				float firstColorFeatherStep = float(0.076) - float(0.071);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(1,0.9117226,0.8465297,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.957614,0.8678106,0.9330387,1).rgb; //* LightColor;
				float shadowMask = 1;
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.161) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.076) - firstColorFeatherStep) * (shadowMask));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.418999,0.2729613,0.2352164,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.65))));
				float _Rimlight_InsideMask_var = saturate(lerp((0.0 + ((_RimLightPower_var - float(0.4)) * (1.0 - 0.0)) / (1.0 - float(0.4))), step(float(0.4), _RimLightPower_var), float(0)));
				...
				float3 _RimLight_var = (saturate((_Set_RimLightMask_var.g + float(0))) * lerp(_LightDirection_MaskOn_var, (_LightDirection_MaskOn_var + (lerp(ApRimColor, (ApRimColor * poiLight.directColor), float(1)) * saturate((lerp((0.0 + ((_ApRimLightPower_var - float(0.4)) * (1.0 - 0.0)) / (1.0 - float(0.4))), step(float(0.4), _ApRimLightPower_var), float(0)) - (saturate(_VertHalfLambert_var) + float(0)))))), float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.4588114,0.2502398,0.317233,1).rgb, float(0)), float4(0.4588114,0.2502398,0.317233,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.4588114,0.2502398,0.317233,1).rgb, float(0)), float4(0.4588114,0.2502398,0.317233,1).a), poiFragData.baseColor, float(0.5)) * col;
				}
				if (float(1) == 2)
				{
