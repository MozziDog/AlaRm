Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/e3b12775ef605a142855cac6e87aabb1"
{
 ...
 #define _RIMSTYLE_UTS2 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				firstShadeMap.rgb *= float4(1,0.9601462,0.9292453,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.9811321,0.9394802,0.9699687,1).rgb; //* LightColor;
				float shadowMask = 1;
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.6792453,0.5591785,0.5222499,1).rgb, float(0)));
 ...
 #define _RIMSTYLE_UTS2 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.7075472,0.5373353,0.5988731,1).rgb, float(0)), float4(0.7075472,0.5373353,0.5988731,1).a);
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, float(0.5));
				if (float(1) == 2)
				{
