Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/b9c6fdbf77bf97e49884e2c1f3050205"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0) * float(0) / 100) * outlineMask * distanceOffset;
				if (float(0) == 2)
				{
			...
			{
				float MainColorFeatherStep = float(0.161) - float(0.013);
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a), poiFragData.baseColor, float(0)) * col;
				}
				if (float(0) == 2)
				{
