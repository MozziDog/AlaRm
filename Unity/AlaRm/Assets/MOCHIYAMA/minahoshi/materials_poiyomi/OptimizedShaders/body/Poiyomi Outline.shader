Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/af827d3ebf25dec42b51d7ace10ab911"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.5448383,0.3208266,0.3418803,1).rgb, float(0)), float4(0.5448383,0.3208266,0.3418803,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.5448383,0.3208266,0.3418803,1).rgb, float(0)), float4(0.5448383,0.3208266,0.3418803,1).a), poiFragData.baseColor, float(0.5)) * col;
				}
				if (float(1) == 2)
				{
