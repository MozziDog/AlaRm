Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/f3d10d172e04b324d9d7cb4c4aadc63c"
{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.6415094,0.4750801,0.5246284,1).rgb, float(0)), float4(0.6415094,0.4750801,0.5246284,1).a);
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, float(0.25));
				if (float(1) == 2)
				{
