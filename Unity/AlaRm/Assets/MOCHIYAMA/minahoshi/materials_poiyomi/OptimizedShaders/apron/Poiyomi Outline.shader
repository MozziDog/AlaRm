Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/d5dcd7e72061a1c41a46e007fdfa44e3"
{
	Properties
	{
...
#define PROP_RIMLIGHT_POWER 0.174
#define PROP_RIMLIGHT_INSIDEMASK 0.983
...
#define PROP_OUTLINETINTMIX 0.25
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3456162,0.2568933,0.2216142,1).rgb, float(0)), float4(0.3456162,0.2568933,0.2216142,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.3456162,0.2568933,0.2216142,1).rgb, float(0)), float4(0.3456162,0.2568933,0.2216142,1).a), poiFragData.baseColor, float(0.25)) * col;
				}
				if (float(1) == 2)
				{
