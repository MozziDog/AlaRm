Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/cb1b01ad93bad2a4c8efb2a5d40e75fc"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.4588114,0.2683191,0.1047679,1).rgb, float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.2140411,0.1129851,0.1734429,1).rgb, float(0)), float4(0.2140411,0.1129851,0.1734429,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.2140411,0.1129851,0.1734429,1).rgb, float(0)), float4(0.2140411,0.1129851,0.1734429,1).a), poiFragData.baseColor, float(0.6)) * col;
				}
				if (float(1) == 2)
				{
