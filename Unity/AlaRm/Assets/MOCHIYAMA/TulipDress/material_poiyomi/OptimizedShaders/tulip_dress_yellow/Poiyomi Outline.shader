Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/9219ef84aa2e60a4eb5cc6e1cbc6c1c4"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.4453062,0.2833244,0.2054933,1).rgb, float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3812781,0.1978161,0.1992524,1).rgb, float(0)), float4(0.3812781,0.1978161,0.1992524,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.3812781,0.1978161,0.1992524,1).rgb, float(0)), float4(0.3812781,0.1978161,0.1992524,1).a), poiFragData.baseColor, float(0.6)) * col;
				}
				if (float(1) == 2)
				{
