Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/b755ba55eed6bd94998104d59e9257ba"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.1729492,0.1224812,0.09939327,1).rgb, float(0)), float4(0.1729492,0.1224812,0.09939327,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.1729492,0.1224812,0.09939327,1).rgb, float(0)), float4(0.1729492,0.1224812,0.09939327,1).a), poiFragData.baseColor, float(0)) * col;
				}
				if (float(1) == 2)
				{
