Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/653a56ed2345278429359cf63c66269c"
{
	Properties
	{
...
#define PROP_VERTEXBARRELMODE 0
#define PROP_VERTEXBARRELWIDTH 0.2
#define PROP_VERTEXBARRELALPHA 0
#define PROP_VERTEXBARRELHEIGHT 0
...
#define PROP_LIGHTINGADDDETAILSHADOWSTRENGTHR 1
#define PROP_LIGHTINGADDDETAILSHADOWSTRENGTHG 0
#define PROP_LIGHTINGADDDETAILSHADOWSTRENGTHB 0
#define PROP_LIGHTINGADDDETAILSHADOWSTRENGTHA 0
...
#define PROP_LIGHTINGMULITLAYERNONLINEAR 1
...
#define PROP_USE_BASEAS1ST 1
#define PROP_2ND_SHADEMAPUV 0
#define PROP_USE_2NDSHADEMAPALPHA_AS_SHADOWMASK 0
#define PROP_2NDSHADEMAPMASK_INVERSE 0
#define PROP_USE_1STAS2ND 1
#define PROP_BASECOLOR_STEP 0.3
#define PROP_BASESHADE_FEATHER 0.013
#define PROP_SHADECOLOR_STEP 0.231
#define PROP_1ST2ND_SHADES_FEATHER 0.504
...
#define PROP_SDFSHADINGTEXTUREUV 0
#define PROP_SDFBLUR 0.1
...
#define PROP_IS_NORMALMAPTORIMLIGHT 0
#define PROP_RIMLIGHTINGINVERT 0
#define PROP_RIMLIGHTCOLORTHEMEINDEX 0
#define PROP_RIMWIDTH 0.55
#define PROP_RIMSHARPNESS 0
#define PROP_RIMPOWER 0.9
#define PROP_RIMSTRENGTH 0
#define PROP_RIMBASECOLORMIX 0
#define PROP_RIMBLENDMODE 2
#define PROP_RIMBRIGHTNESS 1
#define PROP_RIMBLENDSTRENGTH 0.4
#define PROP_IS_LIGHTCOLOR_RIMLIGHT 1
#define PROP_RIMLIGHT_POWER 0.243
#define PROP_RIMLIGHT_INSIDEMASK 0.218
...
#define PROP_RIMENVIROBLUR 0.594
#define PROP_RIMENVIROWIDTH 0.297
#define PROP_RIMENVIROSHARPNESS 0.663
#define PROP_RIMENVIROMINBRIGHTNESS 0
#define PROP_RIMENVIROINTENSITY 0.598
...
#define PROPM_OUTLINECATEGORY 1
#define PROP_ENABLEOUTLINES 1
#define PROP_OUTLINEMODE 1
#define PROP_OUTLINESPACE 0
#define PROP_OUTLINETINTMIX 0.6
...
#define PROP_LINEWIDTH 0.15
#define PROP_LINECOLORTHEMEINDEX 0
...
#define PROP_PPHDR 0
#define PROP_PPPOSTERIZATION 0
#define PROP_PPPOSTERIZATIONAMOUNT 4
...
#define PROPM_START_OUTLINEBLENDING 0
#define PROP_OUTLINESRCBLEND 1
#define PROP_OUTLINEDSTBLEND 0
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float outlineMask = 1;
				outlineMask = poiMax(tex2Dlod(_OutlineMask, float4(poiUV(o.uv[0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0), 0, 0)).rgb);
				if (float(0) == 2)
				{
				...
				float3 offset = outlineNormal * (float(0.15) * float(1) / 100) * outlineMask * distanceOffset;
				if (float(1) == 2)
				{
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(0) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.0009765625,0.0009765625,1024,1024).zw)) * float(0.25);
			...
			{
				float MainColorFeatherStep = float(0.3) - float(0.013);
				float firstColorFeatherStep = float(0.231) - float(0.504);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				firstShadeMap = lerp(firstShadeMap, float4(poiFragData.baseColor, 1), float(1));
				#if defined(PROP_2ND_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				secondShadeMap = lerp(secondShadeMap, firstShadeMap, float(1));
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.3) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.231) - firstColorFeatherStep) * (shadowMask));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.25));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.25), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.25));
				}
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(1) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.5), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			{
				float viewDotNormal = abs(dot(poiCam.viewDir, lerp(poiMesh.normals[0], poiMesh.normals[1], float(0))));
				
				if (float(0))
				{
					viewDotNormal = 1 - viewDotNormal;
				}
				viewDotNormal = pow(viewDotNormal, float(0.9));
				if (float(0) && float(0))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, float(0));
				}
				float rimStrength = float(0);
				float rimWidth = lerp( - .05, 1, float(0.55));
				float blendStrength = float(0.4);
				#if defined(PROP_RIMMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3229691,0.08761136,0.1936618,1).rgb, float(0)), float4(0.3229691,0.08761136,0.1936618,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.3229691,0.08761136,0.1936618,1).rgb, float(0)), float4(0.3229691,0.08761136,0.1936618,1).a);
				#endif
				...
				rim *= float4(0.3229691,0.08761136,0.1936618,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.3229691,0.08761136,0.1936618,1).rgb, float(0)));
				float3 _Is_LightColor_RimLight_var = lerp(rimColor, (rimColor * poiLight.directColor), float(1));
				float _RimArea_var = (1.0 - dot(lerp(poiMesh.normals[0], poiMesh.normals[1], float(0)), poiCam.viewDir));
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.243))));
				float _Rimlight_InsideMask_var = saturate(lerp((0.0 + ((_RimLightPower_var - float(0.218)) * (1.0 - 0.0)) / (1.0 - float(0.218))), step(float(0.218), _RimLightPower_var), float(0)));
				...
				float3 _RimLight_var = (saturate((_Set_RimLightMask_var.g + float(0))) * lerp(_LightDirection_MaskOn_var, (_LightDirection_MaskOn_var + (lerp(ApRimColor, (ApRimColor * poiLight.directColor), float(1)) * saturate((lerp((0.0 + ((_ApRimLightPower_var - float(0.218)) * (1.0 - 0.0)) / (1.0 - float(0.218))), step(float(0.218), _ApRimLightPower_var), float(0)) - (saturate(_VertHalfLambert_var) + float(0)))))), float(0)));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.25))));
				poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.25))));
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(0.4));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(0.4));
				if (float(1))
				{
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = float(1) ? min(float(1.3), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, float(1));
				#endif
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					clip(OutlineMask * float(0.15) - 0.001);
				}
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3341808,0.164666,0.2265122,1).rgb, float(0)), float4(0.3341808,0.164666,0.2265122,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.3341808,0.164666,0.2265122,1).rgb, float(0)), float4(0.3341808,0.164666,0.2265122,1).a), poiFragData.baseColor, float(0.6)) * col;
				}
				if (float(1) == 2)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, poiLight.directColor, float(0)) * col;
				}
				if (float(0))
				{
				...
				poiFragData.emission += poiFragData.baseColor * float(0);
			}
			#endif
			#ifdef VIGNETTE_MASKED
			#ifdef _LIGHTINGMODE_CLOTH
