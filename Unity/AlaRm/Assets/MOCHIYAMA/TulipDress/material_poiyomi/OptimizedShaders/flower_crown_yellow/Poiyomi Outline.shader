Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/5ccd0d3856f1b0c479b9d0f516af3e16"
{
	Properties
	{
...
#define PROPM_OUTLINECATEGORY 0
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.1433049,0.1085262,1).rgb, float(0)), float4(1,0.1433049,0.1085262,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.1433049,0.1085262,1).rgb, float(0)), float4(1,0.1433049,0.1085262,1).a);
				#endif
				...
				rim *= float4(1,0.1433049,0.1085262,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.1433049,0.1085262,1).rgb, float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3572768,0.1934988,0.1908091,1).rgb, float(0)), float4(0.3572768,0.1934988,0.1908091,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.3572768,0.1934988,0.1908091,1).rgb, float(0)), float4(0.3572768,0.1934988,0.1908091,1).a), poiFragData.baseColor, float(0.6)) * col;
				}
				if (float(1) == 2)
				{
