Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/b405f327360651247991871a12873779"
{
	Properties
	{
...
#define PROP_RIMWIDTH 0.75
#define PROP_RIMSHARPNESS 0
#define PROP_RIMPOWER 0.79
...
#define PROP_RIMENVIROBLUR 0.63
#define PROP_RIMENVIROWIDTH 0.4
#define PROP_RIMENVIROSHARPNESS 0.16
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.75));
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.7605246,0.1144353,0.2353367,1).rgb, float(0)), float4(0.7605246,0.1144353,0.2353367,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.7605246,0.1144353,0.2353367,1).rgb, float(0)), float4(0.7605246,0.1144353,0.2353367,1).a);
				#endif
				...
				rim *= float4(0.7605246,0.1144353,0.2353367,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.7605246,0.1144353,0.2353367,1).rgb, float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3691636,0.2484838,0.2849281,1).rgb, float(0)), float4(0.3691636,0.2484838,0.2849281,1).a);
				if (float(1) == 0)
				{
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(0.3691636,0.2484838,0.2849281,1).rgb, float(0)), float4(0.3691636,0.2484838,0.2849281,1).a), poiFragData.baseColor, float(0.6)) * col;
				}
				if (float(1) == 2)
				{
