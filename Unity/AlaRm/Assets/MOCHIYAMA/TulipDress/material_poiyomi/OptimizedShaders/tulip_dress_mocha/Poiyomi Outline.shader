Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/a914fb84516b42b49985c3a50ea24274"
{
	Properties
	{
...
#define PROP_RIMLIGHT_POWER 0.421
#define PROP_RIMLIGHT_INSIDEMASK 0.149
...
#define PROPM_RENDERINGCATEGORY 1
#define PROP_CULL 2
#define PROP_ZTEST 4
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0.16) * float(1) / 100) * outlineMask * distanceOffset;
				if (float(1) == 2)
				{
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(0.57));
				if (float(0))
				{
			...
			void applyDithering(inout PoiFragData poiFragData, in PoiCam poiCam)
			{
				if (float(0))
				{
					poiFragData.alpha = saturate(poiFragData.alpha - (calcDither(poiCam.screenUV) * (1 - poiFragData.alpha) * float(0.2)));
				}
			}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.3341808,0.1790967,0.13843,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.421))));
				float _Rimlight_InsideMask_var = saturate(lerp((0.0 + ((_RimLightPower_var - float(0.149)) * (1.0 - 0.0)) / (1.0 - float(0.149))), step(float(0.149), _RimLightPower_var), float(0)));
				...
				float3 _RimLight_var = (saturate((_Set_RimLightMask_var.g + float(0))) * lerp(_LightDirection_MaskOn_var, (_LightDirection_MaskOn_var + (lerp(ApRimColor, (ApRimColor * poiLight.directColor), float(1)) * saturate((lerp((0.0 + ((_ApRimLightPower_var - float(0.149)) * (1.0 - 0.0)) / (1.0 - float(0.149))), step(float(0.149), _ApRimLightPower_var), float(0)) - (saturate(_VertHalfLambert_var) + float(0)))))), float(0)));
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					clip(OutlineMask * float(0.16) - 0.001);
				}
